import numpy as np
import torch
from segmentation.utils.TopK_loss import TopKLoss
from segmentation.utils.crossentropy import RobustCrossEntropyLoss
from segmentation.utils.nd_softmax import softmax_helper
from segmentation.utils.tensor_utilities import sum_tensor
from torch import nn
from medpy import metric


class DiceAverage(nn.Module):
    ...
    def reset(self):
        self.value = np.asarray([0] * self.class_num, dtype='float32')
        self.avg = np.asarray([0] * self.class_num, dtype='float32')
        self.sum = np.asarray([0] * self.class_num, dtype='float32')
        self.count = 0

    def update(self, mold, logits, targets):
        self.value = DiceAverage.dice(logits, targets)
        self.sum += self.value
        self.count += 1
        self.avg = np.around(np.mean(self.sum / self.count), 4)
