...
class MSIM(nn.Module):
    ...
    def __init__(
        ...
        self.res1 = ResBlock(2, hidden_size, hidden_size, kernel_size=3, stride=1, norm_name="batch")
        self.res2 = ResBlock(2, hidden_size, hidden_size, kernel_size=3, stride=1, norm_name="batch")
        self.conv = nn.Sequential(nn.Dropout2d(0.1, False), nn.Conv2d(hidden_size, hidden_size, 1))
        self.pos_embed = None
        if pos_embed:
...
class CCAB(nn.Module):
    def __init__(self, in_channels, out_channels, mid_channels=None, channel_attention=False):
        ...
        self.in1 = nn.InstanceNorm2d(mid_channels)
...
class Shared_Layer(nn.Module):
    def __init__(self, input_size=14*14, dim=256, proj_size=32, depths=2, num_heads=4,
...
class CMRL_model(nn.Module):
    def __init__(self,
        ...
        self.up11 = UpLayer(base_c * 16, base_c * 8 // factor, bilinear)
        self.up12 = UpLayer(base_c * 8, base_c * 4 // factor, bilinear)
        self.up13 = UpLayer(base_c * 4, base_c * 2 // factor, bilinear)
        self.up14 = UpLayer(base_c * 2, base_c, bilinear)

        self.up21 = UpLayer(base_c * 16, base_c * 8 // factor, bilinear)
        self.up22 = UpLayer(base_c * 8, base_c * 4 // factor, bilinear)
        self.up23 = UpLayer(base_c * 4, base_c * 2 // factor, bilinear)
        self.up24 = UpLayer(base_c * 2, base_c, bilinear)
